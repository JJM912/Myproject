    ...
    if not flipped:
        ...
        if not student_id:
            ...
            if existing:
                ...
                if c1.button("✅ I Know It", use_container_width=True):
                    db.flip_save_response(student_id, word["id"], "know")
                    st.rerun()

                if c2.button("⭐ Save to My Words", use_container_width=True):
...
def render_word_summary(word_id):
    ...
    c2.metric("Saved to My Words", save)
...
def render_teacher_controls():
    ...
    with st.expander("① Add Words", expanded=True):
        ...
        if c1.button("Add Words", use_container_width=True):
            added = 0
        ...
        if c2.button("Load Sample Words", use_container_width=True):
        ...
        if words:
            ...
            for word in words:
                ...
                w1.markdown(f"**{word['word']}**")
                w2.caption(word["meaning"])
